...
void PID_ATune::Cancel() { running = false; }
...
int PID_ATune::Runtime() {
  ...
  lastInputs[0] = refVal;
  if (nLookBack < 9) {  // we don't want to trust the maxes or mins until the
                        // inputs array has been filled
    return 0;
  }
  ...
  if (justchanged && peakCount > 2) {  // we've transitioned.  check if we can
                                       // autotune based on the last peaks
    double avgSeparation = (abs(peaks[peakCount - 1] - peaks[peakCount - 2]) +
                            abs(peaks[peakCount - 2] - peaks[peakCount - 3])) /
                           2;
...
double PID_ATune::GetKp() { return controlType == 1 ? 0.6 * Ku : 0.4 * Ku; }
...
void PID_ATune::SetOutputStep(double Step) { oStep = Step; }

double PID_ATune::GetOutputStep() { return oStep; }
...
void PID_ATune::SetNoiseBand(double Band) { noiseBand = Band; }

double PID_ATune::GetNoiseBand() { return noiseBand; }
